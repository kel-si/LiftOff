...
export default function Quiz(props) {
  ...
  const handleAnswerButtonClick = (isCorrect) => {
	...
	const handleSubmit =(event) => {
		...
		.then((res) => { 
			console.log("complete quiz, from server:", res.data);
    ...
    <div className='quiz-container'>
		<div className='quiz'>
            ...
            <button key={answerOption.answerText.id} className="quiz-button" onClick={() =>handleAnswerButtonClick(answerOption.isCorrect)}>{answerOption.answerText}</button>
            ))}
					</div>
				</>
			)}
